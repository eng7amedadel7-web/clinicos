...
function ProtectedShell() {
  ...
  return <div className="app-shell flex min-h-[100dvh] md:flex-row" dir="ltr"><Sidebar clinicName={session.clinic.name} userName={session.user.fullName} /><div className={`main-content min-h-0 min-w-0 flex-1 ${location === '/inbox' ? 'md:h-[100dvh] md:overflow-hidden' : ''}`} dir="rtl"><WorkspaceToolbar />{location === '/settings' ? <SettingsPage session={session} /> : location === '/patients' ? <LivePatientsPage /> : location === '/appointments' ? <LiveAppointmentsPage /> : location === '/inbox' ? <LiveInboxPage /> : location === '/waitlist' ? <WaitlistPage /> : location === '/follow-ups' ? <FollowUpsPage /> : location === '/no-shows' ? <NoShowsPage /> : location === '/voice-agent' ? <LiveVoiceAgentPage /> : location === '/billing' ? <BillingPage /> : <LiveDashboard session={session} />}</div></div>;
}
