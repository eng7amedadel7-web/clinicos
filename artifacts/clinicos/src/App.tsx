...
import { FollowUpsPage, NoShowsPage, WaitlistPage } from '@/pages/live-operations-pages';
...
function DashboardPage({ session }: { session: { user: { fullName: string }; clinic: { name: string; city: string } } }) {
  return <OperationsDashboard session={session} />;
}
...
function ProtectedShell() {
  ...
  return <div className="app-shell flex min-h-[100dvh] md:flex-row"><Sidebar clinicName={session.clinic.name} userName={session.user.fullName} />{location.startsWith('/appointments/') ? <AppointmentJourneyPage /> : location.startsWith('/patients/') ? <Patient360Page /> : location === '/settings' ? <SettingsPage session={session} /> : location === '/patients' ? <PatientsPage /> : location === '/appointments' ? <AppointmentsPage /> : location === '/inbox' ? <InboxPage /> : location === '/waitlist' ? <WaitlistPage /> : location === '/follow-ups' ? <FollowUpsPage /> : location === '/no-shows' ? <NoShowsPage /> : <DashboardPage session={session} />}</div>;
}
