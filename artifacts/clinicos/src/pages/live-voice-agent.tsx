...
import { Activity, AlertTriangle, BarChart3, CheckCircle2, Clock3, Phone, PhoneCall, RefreshCw, Volume2 } from "lucide-react";
...
export default function LiveVoiceAgentPage() {
  ...
  const durationTotal = calls.reduce((total, call) => total + (typeof call.duration_seconds === "number" ? call.duration_seconds : 0), 0);
  const averageDuration = calls.length ? duration(durationTotal / calls.length) : "—";
  const statusBreakdown = (["completed", "in_progress", "missed", "failed"] as const).map((status) => ({ status, count: calls.filter((call) => call.call_status === status).length }));
    ...
    <section className="mx-auto max-w-[1200px]">
      ...
      {query.isLoading ? <LoadingState /> : query.isError ? <div className="surface flex min-h-[260px] flex-col items-center justify-center p-10 text-center"><AlertTriangle className="mb-3 text-[#a64036]" /><p className="text-sm font-bold text-[#18374d]">تعذر تحميل بيانات الوكيل الصوتي</p><button className="primary-button mt-5" onClick={() => query.refetch()}><RefreshCw size={15} /> إعادة المحاولة</button></div> : (
        <>
          ...
          <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="إجمالي المكالمات" value={String(data?.total ?? 0)} helper="من سجل Voice Agent" icon={Phone} tone="bg-[#dcecf5] text-[#22617d]" /><Metric label="مكالمات مكتملة" value={String(completed)} helper="تم تسجيل نتيجتها" icon={CheckCircle2} tone="bg-[#d9f0e8] text-[#176b58]" /><Metric label="نسبة الإكمال" value={`${data?.total ? Math.round((completed / data.total) * 100) : 0}%`} helper="من إجمالي السجل الحالي" icon={BarChart3} tone="bg-[#e8e1f4] text-[#65518b]" /><Metric label="تحتاج مراجعة" value={String(attention)} helper="لم يتم الرد أو فشلت" icon={AlertTriangle} tone="bg-[#f8dfdc] text-[#a64036]" /></div>
          ...
          <div className="mb-6 grid gap-6 xl:grid-cols-[1.1fr_.9fr]"><section className="surface p-5"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-sm font-extrabold text-[#18374d]">تحليل المكالمات</h2><p className="mt-1 text-[11px] text-[#8496a0]">ملخص السجل المحمّل حاليًا</p></div><BarChart3 size={18} className="text-[#789daa]" /></div><div className="mb-5 grid grid-cols-3 gap-3"><div className="rounded-xl bg-[#f1f7f7] p-3 text-center"><p className="text-lg font-extrabold text-[#18374d]">{averageDuration}</p><p className="mt-1 text-[10px] text-[#8496a0]">متوسط المدة</p></div><div className="rounded-xl bg-[#f1f7f7] p-3 text-center"><p className="text-lg font-extrabold text-[#18374d]">{inbound}</p><p className="mt-1 text-[10px] text-[#8496a0]">واردة</p></div><div className="rounded-xl bg-[#f1f7f7] p-3 text-center"><p className="text-lg font-extrabold text-[#18374d]">{outbound}</p><p className="mt-1 text-[10px] text-[#8496a0]">صادرة</p></div></div><div className="space-y-3">{statusBreakdown.map(({ status, count }) => <div key={status} className="flex items-center gap-3 text-xs"><span className="w-20 shrink-0 font-bold text-[#527080]">{statusLabel(status)}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-[#e9f0f2]"><div className="h-full rounded-full bg-[#5d9caf] transition-all" style={{ width: `${Math.round((count / maxStatusCount) * 100)}%` }} /></div><strong className="w-7 text-left text-[#355467]">{count}</strong></div>)}</div></section><section className="surface p-5"><div className="mb-5 flex items-center gap-2"><Activity size={17} className="text-[#3c7e93]" /><div><h2 className="text-sm font-extrabold text-[#18374d]">قراءة تشغيلية</h2><p className="mt-1 text-[11px] text-[#8496a0]">مؤشرات مساعدة للفريق</p></div></div><div className="space-y-4 text-xs"><div className="flex items-center justify-between rounded-xl bg-[#f1f7f7] p-4"><span className="font-bold text-[#527080]">المكالمات المكتملة</span><strong className="text-[#176b58]">{data?.total ? `${Math.round((completed / data.total) * 100)}%` : "0%"}</strong></div><div className="flex items-center justify-between border-b border-[#edf1f3] pb-4"><span className="font-bold text-[#527080]">نسبة المراجعة</span><strong className="text-[#a64036]">{data?.total ? `${Math.round((attention / data.total) * 100)}%` : "0%"}</strong></div><div className="flex items-center justify-between"><span className="font-bold text-[#527080]">مصدر البيانات</span><span className="font-bold text-[#3c7e93]">Supabase · العيادة الحالية</span></div></div></section></div>
