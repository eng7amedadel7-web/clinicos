...
import { AlertTriangle, Bot, Filter, RefreshCw, Search, Send, ToggleRight, UserRound } from "lucide-react";
    ...
    {data && !inboxQuery.isError ? <div className="grid min-h-[650px] overflow-hidden rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] shadow-[var(--shadow-sm)] lg:grid-cols-[72px_320px_1fr]">
      <aside className="order-1 flex gap-2 border-b border-[hsl(var(--border)/.65)] bg-[hsl(var(--muted)/.32)] p-2 lg:order-none lg:flex-col lg:border-b-0 lg:border-l lg:p-2.5">
        ...
        {channels.map((channel) => {
          ...
          return <button key={channel.key} onClick={() => setChannelType(channel.key)} className={`flex min-w-[58px] flex-1 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[9px] font-bold transition lg:flex-none ${channelType === channel.key ? "bg-[hsl(var(--primary))] text-white shadow-[0_5px_15px_hsl(var(--primary)/.2)]" : "text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]"}`} aria-label={`فتح ${channel.label}`}><span className="grid size-8 place-items-center rounded-lg border border-current/15 text-[9px] font-black">{channel.icon}</span><span className="hidden truncate lg:block">{channel.label}</span><span className="text-[9px] opacity-70">{count}</span></button>;
        })}
      </aside>
      <div className="order-2 border-b border-[hsl(var(--border)/.65)] lg:order-none lg:border-b-0 lg:border-l">
        ...
        <div className="p-3"><div className="relative"><Search size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" /><input value={search} onChange={(event) => setSearch(event.target.value)} data-testid="input-inbox-search" placeholder="ابحث في المحادثات" className="w-full rounded-lg bg-[hsl(var(--muted))] py-2 pr-8 text-[11px] outline-none" /></div></div>
