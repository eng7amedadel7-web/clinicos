...
export function LiveInboxPage() {
  ...
  return <WorkspacePage className="inbox-layout h-full min-h-0 flex-1 max-w-none gap-1 overflow-hidden px-2 py-1 md:px-3 md:py-2" data-testid="page-inbox">
    ...
    <div className="grid grid-cols-2 gap-1 sm:grid-cols-4 lg:grid-cols-5" role="tablist" aria-label="قنوات التواصل">
      <button type="button" role="tab" aria-selected={channelType === "all"} onClick={() => selectChannel("all")} className={`flex items-center justify-center gap-2 rounded-lg border px-2 py-1 text-[10px] font-bold transition active:scale-[.98] ${channelType === "all" ? "border-[hsl(var(--primary))] bg-[hsl(var(--primary))] text-white shadow-[0_7px_18px_hsl(var(--primary)/.2)]" : "border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary)/.45)]"}`}><span className="grid size-5 place-items-center rounded-md border border-current/20 text-[10px]">✦</span><span>كل القنوات</span><span className="rounded-full bg-black/10 px-1 py-0.5 text-[8px]">{items.length}</span></button>
      {inboxChannelMeta.map((meta) => { const count = channelCounts[meta.type] || 0; const configured = channels.some((channel) => channel.type === meta.type && channel.isEnabled !== false); const active = channelType === meta.type; return <button type="button" key={meta.type} role="tab" aria-selected={active} onClick={() => selectChannel(meta.type)} title={configured ? undefined : "القناة غير متصلة أو غير مهيأة"} className={`flex items-center justify-center gap-2 rounded-lg border px-2 py-1 text-[10px] font-bold transition active:scale-[.98] ${active ? "text-white shadow-[0_7px_18px_rgba(0,0,0,.12)]" : "border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary)/.45)]"}`} style={active ? { borderColor: meta.accent, backgroundColor: meta.accent } : undefined}><span className="grid size-5 place-items-center rounded-md" style={{ backgroundColor: active ? "rgba(255,255,255,.18)" : meta.soft }}><ChannelLogo type={meta.type} size={15} /></span><span>{meta.label}</span><span className={`rounded-full px-1 py-0.5 text-[8px] ${active ? "bg-white/20" : "bg-[hsl(var(--muted))]"}`}>{count}</span></button>; })}
    </div>
    <div className="surface flex min-h-0 min-w-0 flex-1 overflow-hidden p-0 lg:grid lg:grid-cols-[350px_minmax(0,1fr)]" dir="ltr" data-testid="inbox-workspace">
      {loading && !items.length ? <div className="p-5 lg:col-span-2"><LoadingState /></div> : error ? <div className="p-5 lg:col-span-2"><ErrorState onRetry={() => load(selectedId || undefined, channelType)} /></div> : <>
        <div className="flex min-h-0 flex-col border-r border-[hsl(var(--border))] bg-[hsl(var(--card))]" dir="rtl">
        ...
        <div className="flex min-h-0 min-w-0 flex-col overflow-hidden" dir="rtl">
