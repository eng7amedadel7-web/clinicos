...
const copy: Record<Lang, Copy> = {
  en: {
    ...
    hero: { eyebrow: 'CLINIC OPERATIONS / ALWAYS ON', title: 'A calmer way to run', accent: 'any clinic.', body: 'Bring patients, appointments, conversations, AI and daily operations into one clear workspace your whole team can trust.', proof: 'Trusted by teams who care about every patient moment.', clinics: '120+ clinics across 18 countries' },
    ...
    method: { eyebrow: 'THE OPERATING LAYER', title: 'The front desk is not a place.', accent: 'It is your clinic’s rhythm.', body: 'When calls, messages and follow-ups scatter across a busy team, the patient feels it. MERUNA SYSTEM brings each moment into one calm, accountable layer.', link: 'Explore the workspace', stat1: '−38%', stat1Body: 'fewer missed appointments', stat2: '24/7', stat2Body: 'coverage for every question' },
    ...
    pricing: { eyebrow: 'START WITH THE WAY YOU WORK', title: 'Begin with one clinic.', accent: 'Scale without the scramble.', body: 'A focused workspace for one location or a connected operating layer for your whole network. We shape the rollout around your reality.', plan: 'PLAN / CLINIC', planTitle: 'Your clinic workspace', features: ['Always-on patient conversations', 'Appointments, reminders and follow-ups', 'Permissions and visibility for every location'], action: 'Talk to our team', plans: [{ key: 'starter', name: 'Starter', description: 'For organized beginnings', monthly: 79, yearly: 64, features: ['One branch', 'Up to 3 users', 'Appointments and patients'] }, { key: 'growth', name: 'Growth', description: 'For clinics on the move', monthly: 179, yearly: 144, features: ['Up to 3 branches', 'Up to 15 users', 'Automation and follow-ups'], popular: true }, { key: 'pro', name: 'Pro', description: 'For medical groups', monthly: 349, yearly: 280, features: ['Unlimited branches', 'Smart voice reception', 'Advanced integrations'] }] },
    ...
    footer: { tagline: 'The operating system for the moments that matter.', product: 'Product', company: 'Company', legal: 'Legal', privacy: 'Privacy', terms: 'Terms', rights: '© 2025 MERUNA SYSTEM. Built for better clinic days.' },
  ...
  ar: {
    ...
    pricing: { eyebrow: 'ابدأ بالطريقة التي تعمل بها', title: 'ابدأ بعيادة واحدة.', accent: 'وتوسع بلا فوضى.', body: 'مساحة عمل مركزة لموقع واحد أو طبقة تشغيل مترابطة لشبكتك كلها. نصمم البداية حول واقعك.', plan: 'الخطة / العيادة', planTitle: 'مساحة عيادتك', features: ['محادثات مرضى حاضرة دائماً', 'مواعيد وتذ��يرات ومتابعات', 'صلاحيات ورؤية لكل فرع'], action: 'تحدث مع فريقنا', plans: [{ key: 'starter', name: 'Starter', description: 'للبدايات المنظمة', monthly: 79, yearly: 64, features: ['فرع واحد', 'حتى ٣ مستخدمين', 'المواعيد والمرضى'] }, { key: 'growth', name: 'Growth', description: 'للعيادات سريعة النمو', monthly: 179, yearly: 144, features: ['حتى ٣ فروع', 'حتى ١٥ مستخدماً', 'الأتمتة والمتابعات'], popular: true }, { key: 'pro', name: 'Pro', description: 'للمجموعات الطبية', monthly: 349, yearly: 280, features: ['فروع بلا حدود', 'الاستقبال الصوتي الذكي', 'التكاملات المتقدمة'] }] },
    ...
    footer: { tagline: 'نظام تشغيل للحظات التي تهم.', product: 'المنتج', company: 'الشركة', legal: 'قانوني', privacy: 'الخصوصية', terms: 'الشروط', rights: '© ٢٠٢٥ MERUNA SYSTEM. لأيام عيادة أفضل.' },
...
function App() {
  ...
  useEffect(() => {
    ...
    document.title = isArabic ? 'MERUNA SYSTEM — تشغيل عيادتك بهدوء' : 'MERUNA SYSTEM — A calmer way to run any clinic';
      ...
      <main id="top">
        <section className="cf-hero relative overflow-hidden pb-24 pt-32 md:pb-32 md:pt-44">
          ...
          <div className="cf-container relative grid items-center gap-16 lg:grid-cols-[.84fr_1.16fr] lg:gap-20">
            ...
            <ProductPreview t={t} activeTab={activeTab} selectTab={selectTab} currentMessages={currentMessages} messageInput={messageInput} setMessageInput={setMessageInput} sendMessage={sendMessage} sampleIndex={sampleIndex} setSampleIndex={(index) => { cancelPendingReply(); setSampleIndex(index); setMessages([]); }} isThinking={isThinking} autoPlay={autoPlay} setAutoPlay={setAutoPlay} />
          </div>
        </section>
        ...
        <section id="pricing" className="cf-blue-soft py-24 md:py-28"><div className="cf-container"><div className="mb-12 max-w-[700px]"><Eyebrow>{t.pricing.eyebrow}</Eyebrow><h2 className="cf-display text-[38px] font-extrabold leading-[1.12] tracking-[-.06em] md:text-[51px]">{t.pricing.title}<br /><span className="text-[hsl(var(--primary))]">{t.pricing.accent}</span></h2><p className="cf-body-muted mt-6 max-w-[560px] text-[15px] leading-[1.9]">{t.pricing.body}</p></div><div className="grid gap-4 lg:grid-cols-3">{t.pricing.plans.map((plan) => <article key={plan.key} className={`cf-card relative flex flex-col rounded-[26px] border p-6 shadow-[var(--shadow-soft)] md:p-7 ${plan.popular ? 'border-[hsl(var(--primary)/.45)] shadow-[0_18px_45px_hsl(var(--primary)/.13)]' : ''}`}>{plan.popular && <span className="absolute -top-3 right-6 rounded-full bg-[hsl(var(--primary))] px-3 py-1 text-[10px] font-bold text-white">{isArabic ? 'الأكثر اختياراً' : 'Most chosen'}</span>}<div className="flex items-start justify-between border-b border-[hsl(var(--border))] pb-5"><div><span className="cf-mono text-[9px] text-[hsl(var(--primary))]">{t.pricing.plan}</span><h3 className="cf-display mt-2 text-[23px] font-extrabold">{plan.name}</h3><p className="mt-1 text-[11px] text-[hsl(var(--muted-foreground))]">{plan.description}</p></div><Sparkles className={`size-6 ${plan.popular ? 'text-[hsl(var(--primary))]' : 'text-[hsl(var(--accent))]'}`} /></div><div className="mt-6 flex items-end gap-2"><strong className="cf-mono text-[38px] font-bold text-[hsl(var(--foreground))]">${plan.monthly}</strong><span className="pb-1 text-[11px] text-[hsl(var(--muted-foreground))]">/ {isArabic ? 'شهرياً' : 'month'}</span></div><p className="mt-1 text-[10px] text-[hsl(var(--muted-foreground))]">{isArabic ? `أو $${plan.yearly} شهرياً عند الدفع السنوي` : `or $${plan.yearly} / month billed yearly`}</p><div className="my-6 flex flex-1 flex-col gap-3">{plan.features.map((item) => <div key={item} className="flex items-center gap-3 text-[12px] font-bold"><CheckCircle2 className="size-4 shrink-0 text-[hsl(var(--primary))]" />{item}</div>)}</div><PrimaryButton onClick={openDemo} testId={`button-pricing-${plan.key}`} className="w-full justify-center">{t.pricing.action}</PrimaryButton></article>)}</div></div></section>
        ...
        <section className="cf-tint meruna-final-cta relative overflow-hidden py-28 text-center md:py-36"><div className="absolute right-[-80px] top-[-120px] size-[350px] rounded-full border-[55px] border-[hsl(var(--primary)/.1)]" /><div className="absolute bottom-[-160px] left-[-100px] size-[390px] rounded-full border-[65px] border-[hsl(var(--accent)/.1)]" /><div className="cf-container relative"><Eyebrow>{t.cta.eyebrow}</Eyebrow><h2 className="cf-display mx-auto max-w-[760px] text-[40px] font-extrabold leading-[1.12] tracking-[-.06em] md:text-[58px]">{t.cta.title}<br /><span className="text-[hsl(var(--primary))]">{t.cta.accent}</span></h2><p className="cf-body-muted mx-auto mt-6 max-w-[510px] text-[15px] leading-[1.9]">{t.cta.body}</p><div className="mt-9 flex flex-wrap justify-center gap-3"><PrimaryButton onClick={openDemo} testId="button-final-demo">{t.actions.demo}</PrimaryButton><button type="button" onClick={() => setToast(t.cta.details)} data-testid="button-final-contact" className="cf-button-ghost inline-flex items-center justify-center rounded-full px-5 py-3 text-[12px] font-bold transition-colors">{t.cta.details}</button></div></div></section>
      </main>

      <footer className="cf-card border-t py-12"><div className="cf-container grid gap-10 md:grid-cols-[1.3fr_.7fr_.8fr_.9fr]"><div><BrandMark /><p className="cf-body-muted mt-4 max-w-[260px] text-[11px] leading-[1.8]">{t.footer.tagline}</p><p className="cf-mono mt-5 text-[9px] text-[hsl(var(--muted-foreground))]">MERUNA — Egypt</p></div><div><strong className="mb-4 block text-[11px]">{t.footer.product}</strong><nav className="flex flex-col gap-3 text-[11px] text-[hsl(var(--muted-foreground))]"><a href="#capabilities">{t.nav.capabilities}</a><a href="#voice">{t.nav.voice}</a><a href="#pricing">{t.nav.pricing}</a></nav></div><div><strong className="mb-4 block text-[11px]">{isArabic ? 'الدعم' : 'Support'}</strong><nav className="flex flex-col gap-3 text-[11px] text-[hsl(var(--muted-foreground))]"><a href="/contact">{isArabic ? 'تواصل معنا' : 'Contact us'}</a><a href="mailto:meruna.tech@gmail.com">meruna.tech@gmail.com</a><a href="/refund-policy">{isArabic ? 'الاسترداد خلال 30 يوماً' : '30-day refunds'}</a></nav></div><div><strong className="mb-4 block text-[11px]">{t.footer.legal}</strong><nav className="flex flex-col gap-3 text-[11px] text-[hsl(var(--muted-foreground))]"><a href="/privacy-policy">{t.footer.privacy}</a><a href="/terms">{t.footer.terms}</a><a href="/cookie-policy">{isArabic ? 'ملفات الارتباط' : 'Cookie policy'}</a></nav></div></div><div className="cf-container mt-10 border-t border-[hsl(var(--border))] pt-6"><p className="cf-mono text-[8px] text-[hsl(var(--muted-foreground))]">{isArabic ? '© 2026 MERUNA SYSTEM. جميع الحقوق محفوظة.' : '© 2026 MERUNA SYSTEM. All rights reserved.'}</p></div></footer>
...
function ProductPreview({ t, activeTab, selectTab, currentMessages, messageInput, setMessageInput, sendMessage, sampleIndex, setSampleIndex, isThinking, autoPlay, setAutoPlay }: { t: Copy; activeTab: DemoTab; selectTab: (tab: DemoTab) => void; currentMessages: ChatMessage[]; messageInput: string; setMessageInput: (value: string) => void; sendMessage: (event?: FormEvent) => void; sampleIndex: number; setSampleIndex: (index: number) => void; isThinking: boolean; autoPlay: boolean; setAutoPlay: (playing: boolean) => void }) {
  return <div className="meruna-product-preview order-1 relative lg:order-2"><div className="absolute -right-4 top-[-24px] z-10 hidden items-center gap-2 rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--card)/.9)] px-3 py-2 text-[9px] font-bold shadow-[var(--shadow-soft)] backdrop-blur sm:flex"><span className="size-2 rounded-full bg-[hsl(var(--accent))]" />{t.demo.newMessage}</div><div className="meruna-sim-shell cf-dash-shadow overflow-hidden rounded-[25px] border border-white/[.12] bg-[hsl(var(--panel))] text-[hsl(214_33%_94%)]"><div className="meruna-sim-topbar flex items-center justify-between border-b border-white/[.1] px-5 py-4"><div className="flex items-center gap-2" dir="ltr"><span className="size-2 rounded-full bg-[#f07161]" /><span className="size-2 rounded-full bg-[#d5aa5f]" /><span className="size-2 rounded-full bg-[#69c49a]" /></div><div className="flex items-center gap-3 text-[9px] text-[hsl(214_25%_67%)]"><span className="flex items-center gap-1.5"><span className="size-1.5 animate-pulse rounded-full bg-[hsl(var(--accent))]" />{t.chat.live} · {t.chat.tabs[activeTab]}</span><button type="button" onClick={() => setAutoPlay(!autoPlay)} className="meruna-sim-playback rounded-full border border-current px-2 py-1 transition-colors hover:text-[hsl(var(--primary))]" aria-label={autoPlay ? t.chat.pause : t.chat.resume}>{autoPlay ? 'Ⅱ' : '▶'}</button></div></div><div className="meruna-sim-toolbar border-b border-white/[.1] px-4 py-3"><div className="mb-3 flex items-center justify-between"><span className="text-[10px] font-bold text-[hsl(214_25%_76%)]">{t.chat.workspace}</span><Settings2 className="size-3.5 text-[hsl(214_25%_55%)]" /></div><div className="grid grid-cols-4 gap-1.5">{(Object.keys(t.chat.tabs) as DemoTab[]).map((tab) => { const Icon = tabIcons[tab]; return <button key={tab} type="button" onClick={() => selectTab(tab)} data-testid={`button-demo-tab-${tab}`} className={`flex flex-col items-center gap-1 rounded-xl px-2 py-2 text-[9px] font-bold transition-colors ${activeTab === tab ? 'bg-[hsl(var(--primary))] text-white' : 'text-[hsl(214_25%_68%)] hover:bg-white/[.07]'}`}><Icon className="size-3.5" />{t.chat.tabs[tab]}</button>; })}</div></div><div className="meruna-sim-body grid min-h-[390px] sm:grid-cols-[.72fr_1.28fr]"><aside className="meruna-sim-sidebar hidden border-e border-white/[.1] p-4 sm:block"><div className="mb-4 flex items-center justify-between"><span className="text-[10px] font-bold">{t.chat.patients}</span><Plus className="size-3 text-[hsl(var(--primary))]" /></div><div className="mb-4 flex items-center gap-2 rounded-lg bg-white/[.06] px-2.5 py-2 text-[9px] text-[hsl(214_25%_55%)]"><MessageCircle className="size-3" />{t.chat.search}</div><div className="space-y-1">{t.chat.samplePatients.map((patient, index) => <button type="button" key={patient.name} onClick={() => { setSampleIndex(index); }} data-testid={`button-patient-${index}`} className={`w-full rounded-xl p-2.5 text-start transition-colors ${sampleIndex === index ? 'bg-white/[.1]' : 'hover:bg-white/[.05]'}`}><div className="flex items-center justify-between"><span className="text-[10px] font-bold">{patient.name}</span><span className="cf-mono text-[8px] text-[hsl(214_25%_52%)]">{patient.time}</span></div><span className="mt-1 block truncate text-[9px] text-[hsl(214_25%_58%)]">{patient.detail}</span></button>)}</div></aside><div className="meruna-sim-content flex min-w-0 flex-col p-4 sm:p-5"><div className="mb-5 flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-full bg-[hsl(var(--primary))] text-white"><Bot className="size-4" /></span><span><strong className="block text-[10px]">{t.chat.agentName}</strong><small className="text-[8px] text-[hsl(var(--accent))]">{t.chat.agentStatus}</small></span></div><span className="rounded-full bg-[hsl(var(--accent)/.13)] px-2 py-1 text-[8px] font-bold text-[hsl(var(--accent))]">{t.chat.bookingReady}</span></div><div className="cf-chat-scroll flex min-h-[190px] flex-1 flex-col gap-3 overflow-y-auto pr-1">{currentMessages.map((message, index) => <div key={`${message.from}-${index}`} data-testid={`text-chat-message-${index}`} className={`max-w-[88%] rounded-2xl px-3.5 py-2.5 text-[10px] leading-[1.7] ${message.from === 'agent' ? 'self-start rounded-bl-sm bg-[hsl(var(--primary))] text-white' : 'self-end rounded-br-sm bg-white/[.09] text-[hsl(214_28%_84%)]'}`}>{message.text}</div>)}{isThinking && <div className="meruna-thinking self-start rounded-2xl rounded-bl-sm px-3.5 py-2.5 text-[10px]">{t.chat.agentName} <span aria-hidden="true">•••</span></div>}</div><form onSubmit={sendMessage} className="mt-4 flex items-center gap-2 rounded-xl border border-white/[.12] bg-white/[.04] p-1.5"><input value={messageInput} onChange={(event) => setMessageInput(event.target.value)} data-testid="input-chat-message" aria-label={t.chat.input} placeholder={t.chat.input} className="min-w-0 flex-1 bg-transparent px-2 text-[10px] text-white outline-none placeholder:text-[hsl(214_25%_50%)]" /><button type="submit" data-testid="button-send-chat" aria-label={t.actions.send} className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-[hsl(var(--primary))] text-white transition-transform hover:scale-105"><Send className="size-3.5" /></button></form></div></div></div></div>;
}

function VoicePreview({ t, onPlay }: { t: Copy; onPlay: () => void }) {
  return <div className="meruna-voice-preview relative"><div className="absolute -inset-5 rounded-[42px] border border-[hsl(var(--primary)/.15)]" /><div className="cf-card relative overflow-hidden rounded-[30px] border p-5 shadow-[var(--shadow-soft)] md:p-7"><div className="mb-6 flex items-center justify-between"><span className="cf-mono text-[9px] text-[hsl(var(--muted-foreground))]">VOICE PREVIEW / GLOBAL</span><span className="flex items-center gap-2 text-[9px] font-bold text-[hsl(var(--accent))]"><span className="size-2 animate-pulse rounded-full bg-[hsl(var(--accent))]" />{t.voice.connected}</span></div><div className="rounded-[22px] bg-[hsl(var(--tint))] p-5"><div className="mb-7 flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-full bg-[hsl(var(--navy))] text-[hsl(var(--primary))]"><Phone className="size-5" /></span><div><strong className="block text-[12px]">{t.voice.assistant}</strong><span className="text-[9px] text-[hsl(var(--muted-foreground))]">{t.voice.listening}</span></div><div className="ms-auto flex items-end gap-1" dir="ltr">{[12, 23, 17, 30, 20, 13, 26].map((height, index) => <span key={index} className="w-1 rounded-full bg-[hsl(var(--primary))]" style={{ height }} />)}</div></div><div className="space-y-3"><div className="meruna-voice-bubble meruna-voice-agent max-w-[82%] rounded-2xl rounded-bl-sm bg-[hsl(var(--navy))] px-4 py-3 text-[11px] leading-[1.75] text-[hsl(214_33%_94%)]">{t.voice.listening}</div><div className="meruna-voice-bubble meruna-voice-patient ms-auto max-w-[82%] rounded-2xl rounded-br-sm bg-[hsl(var(--card))] px-4 py-3 text-[11px] leading-[1.75] text-[hsl(var(--muted-foreground))]">{isArabicText(t) ? 'أحتاج مساعدة في موعدي القادم.' : 'I need help with my next appointment.'}</div><div className="meruna-voice-bubble meruna-voice-agent max-w-[82%] rounded-2xl rounded-bl-sm bg-[hsl(var(--navy))] px-4 py-3 text-[11px] leading-[1.75] text-[hsl(214_33%_94%)]">{isArabicText(t) ? 'بالطبع. سأراجع الموعد وأخبرك بالخطوة التالية.' : 'Of course. I’ll check your appointment and share the next step.'}</div></div></div><div className="mt-5 flex items-center justify-between rounded-2xl border border-[hsl(var(--border))] px-4 py-3"><span className="text-[10px] text-[hsl(var(--muted-foreground))]">{t.voice.placeholder}</span><button type="button" onClick={onPlay} data-testid="button-voice-play" aria-label={t.actions.play} className="flex size-9 items-center justify-center rounded-full bg-[hsl(var(--primary))] text-white transition-transform hover:scale-105"><Play className="size-3 fill-current" /></button></div></div></div>;
}
