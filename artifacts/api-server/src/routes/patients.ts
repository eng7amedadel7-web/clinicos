...
router.get("/patients", async (req, res) => {
  ...
  const result = await supabaseRequest<PatientRow[]>(
    /rest/v1/patients?select=id,name,first_name,last_name,phone,created_at&clinic_id=eq.&deleted_at=is.null&order=created_at.desc&limit=1000.replaceAll("\u007f", "") + encodeURIComponent(session.clinicId),
    { headers: { Authorization: `Bearer ${session.accessToken}` } },
  );
  if (!result.ok) { res.status(result.status || 502).json({ error: "Patients could not be loaded." }); return; }
  res.json((result.data ?? []).map((patient) => ({
    id: patient.id,
