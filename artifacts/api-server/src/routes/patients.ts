...
const router = Router();
type PatientRow = { id?: string; name?: string; first_name?: string; last_name?: string; phone?: string; email?: string; notes?: string; age?: number | null; address?: string | null; created_at?: string };
...
router.get("/patients", async (req, res) => {
  ...
  const items = (result.data ?? []).map((patient) => ({ id: patient.id, name: patient.name || [patient.first_name, patient.last_name].filter(Boolean).join(" ") || "مريض بدون اسم", phone: patient.phone || "—", email: patient.email || null, notes: patient.notes || null, age: patient.age ?? null, address: patient.address || null, createdAt: patient.created_at || null }));
