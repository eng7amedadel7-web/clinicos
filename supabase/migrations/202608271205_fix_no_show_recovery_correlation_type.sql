-- Fix correlation_id type mismatch when bridging no-show outcomes into notification events.
-- The source no_show_cases value is text while notification_domain_events expects uuid.
...
BEGIN
  ...
  IF v_final_no_response THEN
    ...
    VALUES (
      ...
      now(),
      CASE
        WHEN c.correlation_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          THEN c.correlation_id::uuid
        ELSE NULL
      END,
      'no_show_bridge'
    )
